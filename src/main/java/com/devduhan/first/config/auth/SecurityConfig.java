package com.devduhan.first.config.auth; //시큐리티 관련 클래스는 모두 이곳에 담음

import com.devduhan.first.domain.user.Role;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.csrf().disable().headers().frameOptions().disable() //h2console화면을 사용하기 위해 해당옵션들을 disable
                .and().authorizeRequests() //URL별 권한 관리를 설정하는 옵션, 선언되어야만 andMatchers 옵션 사용 가능
                /**
                 * andMatchers
                 * - 권한 관리 대상을 지정하는 옵션
                ...
                .and()
                .logout().logoutSuccessUrl("/") //로그아웃 기능에 대한 설정 진입점, 로그아웃 성공 시 / 주소로 이동
                .and()
